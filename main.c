...
void* get_page(Pager* pager, uint32_t page_num) {
  if (page_num >= TABLE_MAX_PAGES) {
    printf(
      "Tried to fetch page number out of bounds. %u >= %u\n",
      page_num,
      TABLE_MAX_PAGES
    );
    exit(EXIT_FAILURE);
...
ExecuteResult execute_insert(
  ...
  Cursor* scan_cursor = table_start(table);

  Row existing_row;

  while (!scan_cursor->end_of_table) {
    deserialize_row(
        cursor_value(scan_cursor),
        &existing_row
    );
